...
from utils import DEV, Env
import glob

warnings.filterwarnings("ignore", category=DeprecationWarning)


# svg#id.cls-[1rem]: filename
PARSE_SVG_PATH_RE = re.compile(
    r"svg(?:#(?P<id>[A-Za-z\-0-9]+))?(?P<cls>(?:\.[!A-Za-z\-/0-9\[\]:${}]+)+)? : (?P<path>[A-Za-z\-0-9/_\.]+)"
)
...
def parse_md_path(indent_level, current_line, matched, source, syntax):
    ...
    md_file = f"public/markdown/{path}.md"
...
def parse_svg_path(indent_level, current_line, matched, source, syntax):
    ...
    svg_file = f"public/svg/{path}.svg"
